  ...
  if (editors.length === 1) {
    ...
    return (
      <div className="flex items-center space-x-2 mt-2">
        ...
        {showNames && (
          <Link href={`/users/${editor.uid}`}>
            <a className="text-sm text-gray-700 dark:text-gray-300 hover:underline">
              {editor.displayName}
            </a>
          </Link>
        )}
  ...
  return (
    <div className="flex items-center space-x-2 mt-2">
      {displayEditors.map((editor) => (
        <Link key={editor.uid} href={`/users/${editor.uid}`}>
          <a>
            {editor.avatarUrl ? (
              <img
                src={editor.avatarUrl}
              ...
              <ul className="space-y-4">
                {editors.map((editor) => (
                  <li key={editor.uid} className="flex items-center space-x-3">
                    ...
                    {showNames && (
                      <Link href={`/users/${editor.uid}`}>
                        <a className="text-lg font-medium text-gray-800 dark:text-gray-100 hover:underline">
                          {editor.displayName}
                        </a>
                      </Link>
                    )}
